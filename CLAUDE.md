...
# Step 1 — build @chora/core (signal math single source of truth)
pnpm build
# Step 2 — generate training data from the TypeScript PulseGenerator
#           (signal math lives ONLY in generator.ts; train.py just consumes CSV)
node scripts/generate-training-data.mjs
# Outputs: packages/training/data/pulses.csv  (200 000 rows, ~14 MB)
# Step 3 — train and export the ONNX model
...
**ONNX model** (`packages/training/train.py`) is a GRU (hidden=16, 1 layer) trained on 200k synthetic pulses with a sliding window of 32. Training data is generated by `scripts/generate-training-data.mjs`, which calls the TypeScript `PulseGenerator` — making `generator.ts` the **single source of truth** for signal math (no Python duplication). Must be trained and placed at `models/predictive_model.onnx` before running. The CLI gracefully degrades to Layer 0-only if the model file is missing.

**LLM integration** supports `OllamaProvider` (default) and `LlamaProvider`. Select provider via `LLM_PROVIDER=ollama|llama` env var. The connection endpoint defaults to local address (`http://localhost:11434/api/generate` or `http://localhost:8080/completion`), but can be overridden globally using `LLM_ENDPOINT` env var (or provider-specific `OLLAMA_ENDPOINT` / `LLAMA_ENDPOINT`). Model defaults to `hf.co/unsloth/Qwen3-4B-Instruct-2507-GGUF:Q4_K_M`; override via `OLLAMA_MODEL`. Prompt language defaults to `en`; override via `PROMPT_LANG=ja`. The LLM returns JSON with `use_existing_name | new_name`, `description`, and `confidence`.
